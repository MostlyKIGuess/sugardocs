---
{"dg-publish":true,"permalink":"/readme/","tags":["gardenEntry"],"noteIcon":""}
---


# Sugar Labs

Welcome to documentation for contributors and developers.

## Getting started

* [Code of Conduct](src/code-of-conduct)
* [How can I help?](src/how-can-i-help)
* [Contribute code](src/contributing)
* [Programming Languages](src/languages)
* [Contact](src/contact)

## Design

* [Human Interface Guidelines](src/hig)
* [Sugar Iconify](src/sugar-iconify)

## Sugar Desktop

* [Setup a development environment](https://github.com/sugarlabs/sugar/blob/master/docs/development-environment)
* [Write your own Sugar desktop activity](src/desktop-activity)
* [Python code style](src/python-style)
* [Memory leaks](src/memory-leaks)
* [GTK 3 Porting Guide](src/gtk3-porting-guide)
* [Python 3 Porting Guide](src/python-porting-guide)
* [Python Coverage Guide](src/python-coverage-guide)
* [Guide to installing Sugar on Debian](src/sugar-on-debian)

## Sugar Web

* [Write your own Sugar web activity](src/web-activity)
* [Architecture](src/web-architecture)
* [JavaScript style](src/javascript-style)
* [Web code style](src/web-style)
* [Components showcase](http://sugarlabs.github.io/sugar-web-samples/)

## Sugar Android

* [Sugarizer](https://github.com/llaske/sugarizer)
* [Web activity](src/android)

## Documentation

* [Improving this documentation](src/docs)

## Translation
* [Weblate](https://weblate.sugarlabs.org)
